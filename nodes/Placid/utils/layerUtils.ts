...
/**
 * Generates the unified layer configuration fields that can be used across image, video, and PDF operations
 ...
 */
export function createLayerConfigurationFields(
	...
	// Get all available layer types from the layer properties system
	const allLayerTypes = getAllLayerTypes();

	// Generate property fields for each layer type dynamically
	const propertyFields: INodeProperties[] = [];
	
	// Create property selection fields for each layer type
...
/**
 * Interface for legacy layer data structure (for backward compatibility)
 */
...
/**
 * Processes legacy layer data (for backward compatibility with PDF/Video)
 * @param textLayers - Array of text layer data
 * @param pictureLayers - Array of picture layer data
 * @returns Object with layer configurations for Placid API
 */
export function processLegacyLayers(
	textLayers: LegacyLayerData[] = [],
	pictureLayers: LegacyLayerData[] = []
): { [key: string]: any } {
	const processedLayers: { [key: string]: any } = {};
...
/**
 * Creates layer fields for nested collections (like PDF pages or video clips)
 * @param resourceType - The resource type for feature customization
 * @returns Array of layer configuration fields without the outer collection wrapper
 */
export function createNestedLayerFields(resourceType: ResourceType): INodeProperties[] {
	// Get all available layer types from the layer properties system
	const allLayerTypes = getAllLayerTypes();
